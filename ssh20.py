...
# Try to retrieve the start-up configuration multiple times with a delay
for _ in range(3):
    session.sendline('show startup-config')
    if wait_for_prompt(session, prompt, timeout=30):  # Increased timeout for retrieving startup-config
        break
    else:
        print('--- Retrying to retrieve start-up configuration...')
        time.sleep(5)

else:
    print('--- FAILURE! Unable to retrieve start-up configuration after multiple attempts')
    exit()
